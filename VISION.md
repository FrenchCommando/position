...
- **Joining is asynchronous.** Relays are the mailbox; newcomer and accepter never need to be online together:
    ...
    **Implemented today (2026-06-13):** the member pastes the newcomer's key and taps Add; the newcomer's app auto-adopts the wrapped key. The verify-the-sender guard — only adopt a group key sent by a key you already hold for that friend — is noted but not built, so adoption currently trusts any key sent to you.
