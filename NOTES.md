...
- **Key distribution** — NIP-44-wrap the group key to each member's pubkey, published as addressable events. Adoption is **trust-on-first-use**: the first key (when you have no group) is trusted; afterwards only a sender already in your roster can hand you a new key, so a stranger can't push a higher-epoch key to hijack you. The stronger pre-shared-inviter guard is still not built (TOFU leaves a first-use window).
